...
class Sprite:
    ...
    def collide (self, other_object):
        global object_collided, my_ship, distance
...
def group_collide(group, other_object):
    ...
    for grp in set(group):
        if grp.collide(other_object):
            a_explosion=Sprite(grp.get_position(), [0,0], 0, 0, explosion_image, explosion_info, explosion_sound)
            explosion_group.add(a_explosion)
            remove_group.add(grp)
            collide=True
...
def reset_game():
    global started, score, time,lives, rock_group
    started=False
    score=0
    time=0
    lives=3
...
def rock_spawner():
    ...
    rock_vel = [random.random() * 1.5, random.random() * 1.5]
...
def process_sprite_group(setsprites,canvas):
    remove_set=set()
    for sprite in setsprites:
        sprite.draw(canvas)
        if (sprite.update()):
            remove_set.add(sprite)
            #p.update()
    setsprites.difference_update(remove_set)
...
##a_explosion=Sprite([100,100], [200,200], 0, 0, explosion_image, explosion_info, explosion_sound)
rock_group=set()
missile_group=set()
explosion_group=set()
